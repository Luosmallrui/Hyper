...
- `area` 可以传商圈中文名或数字 ID。
- `tags` / `tag_ids` 使用 `GET /api/v1/tags` 返回的动态标签 ID。多个值以逗号分隔，目标必须同时包含所有所选标签才会返回。
...
- `distance` 单位 km。
- 未传 `lat/lng` 时不做距离过滤。
- 票务活动、场地和旧派对均已按 `tags` / `tag_ids` 做动态优惠标签硬过滤，并返回真实 `tag_ids`、`tags`；`discount_tags` 保留为兼容的名称数组。
...
- 订单过期后后端会自动取消、关闭待支付流水、返还抵扣积分，并返回错误。
