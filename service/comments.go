...
func (cs *CommentsService) GetComments(ctx context.Context, postId uint64, page, pageSize int) ([]*models.Comments, int64, error) {
	...
	// 分页获取评论
	if err := cs.DB.WithContext(ctx).Model(&models.Comments{}).
		Where("post_id = ? AND root_id = 0 AND status = 1", postId).
...
// 删除
func (cs *CommentsService) DeleteComment(ctx context.Context, commentID, userID uint64) error {
	if commentID == 0 {
		return errors.New("要删除的评论ID无效")
	}
	if userID == 0 {
	...
	if err := cs.DB.WithContext(ctx).
		...
		First(&comment).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return errors.New("评论不存在: " + err.Error())
		}
		return errors.New("查询评论失败: " + err.Error())
	}
	if comment.Status == 0 {
	...
	return cs.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&models.Comments{}).
			...
			return errors.New("删除评论失败: " + err.Error())
		}
		// 如果是顶级评论，删除其所有回复
		if comment.RootID == 0 {
			if err := tx.Model(&models.Comments{}).
				...
				Update("status", 0).Error; err != nil {
				return errors.New("删除评论回复失败: " + err.Error())
			}
		}
		return nil
	})
...
// 点赞
func (cs *CommentsService) LikeComment(ctx context.Context, commentID, userID uint64) error {
	...
	if err := cs.DB.WithContext(ctx).
		...
		return errors.New("已经点赞该评论")
	} else if err != gorm.ErrRecordNotFound {
		return errors.New("查询点赞记录失败: " + err.Error())
	}
	...
	return cs.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Create(&commentlike).Error; err != nil {
			return errors.New("创建点赞记录失败: " + err.Error())
		}

		if err := tx.Model(&models.Comments{}).
			Where("id = ?", commentID).
			UpdateColumn("like_count", gorm.Expr("like_count + ?", 1)).Error; err != nil {
			return errors.New("更新评论点赞数失败: " + err.Error())
		}
		return nil
	})
...
// 取消点赞
func (cs *CommentsService) UnlikeComment(ctx context.Context, commentID, userID uint64) error {
	if commentID == 0 {
		return errors.New("评论ID无效")
	}
	if userID == 0 {
	...
	return cs.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&models.CommentLike{}).
			Where("comment_id = ? AND user_id = ?", commentID, userID).
			Delete(&models.CommentLike{}).Error; err != nil {
			return errors.New("删除点赞记录失败: " + err.Error())
		}
